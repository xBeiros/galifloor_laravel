<template>
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow dark:shadow-gray-900">
        <h3 class="text-lg font-semibold mb-4 text-gray-900 dark:text-white">{{ t('vehicle.show.upload_document') }}</h3>
        ...
        <form @submit.prevent="uploadDocument" class="space-y-4">
            ...
            <div v-if="selectedFile" class="flex items-center space-x-2">
                ...
                <span class="text-xs text-gray-400 dark:text-gray-500">({{ formatFileSize(selectedFile.size) }})</span>
            </div>
        ...
        <div v-if="message" class="mt-4">
            <div v-if="messageType === 'success'" class="bg-green-50 dark:bg-green-900 border border-green-200 dark:border-green-700 text-green-700 dark:text-green-300 px-4 py-3 rounded">
                {{ message }}
            </div>
            <div v-else-if="messageType === 'error'" class="bg-red-50 dark:bg-red-900 border border-red-200 dark:border-red-700 text-red-700 dark:text-red-300 px-4 py-3 rounded">
                {{ message }}
            </div>
        </div>
    </div>
...
const uploadDocument = () => {
    ...
    form.post(`/vehicle/${props.vehicleId}/upload-document`, {
        preserveScroll: true,
        onSuccess: () => {
